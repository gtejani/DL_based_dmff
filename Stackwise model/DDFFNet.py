...
class DDFFNet(nn.Module):
    ...
    def forward(self, images):
        ...
        image = images.permute(0,2,1,3,4)
        image_features = self.autoencoder(image)
        image_features = image_features.permute(0,2,1,3,4)
        #Encode stacks in feature dimension again
        image_features = image_features.view(image_features.shape[0], 10, image_features.shape[3],image_features.shape[4])
...
class DDFFAutoEncoder(nn.Module):
    ...
    def __init__(self, output_dims, cc1_enabled, cc2_enabled, cc3_enabled, cc4_enabled, cc5_enabled, bias=False):
        ...
        #Encoder
        self.conv1_1 = nn.Conv3d(3, 64, 3, padding=1, bias=bias)
        self.conv1_1_bn = nn.BatchNorm3d(64, eps=0.001)
        self.conv1_2 = nn.Conv3d(64, 64, 3, padding=1, bias=bias)
        self.conv1_2_bn = nn.BatchNorm3d(64, eps=0.001)
        self.pool1 = nn.MaxPool3d((1,2,2), stride=(1,2,2))
        self.conv2_1 = nn.Conv3d(64, 128, 3, padding=1, bias=bias)
        self.conv2_1_bn = nn.BatchNorm3d(128, eps=0.001)
        self.conv2_2 = nn.Conv3d(128, 128 , 3, padding=1, bias=bias)
        self.conv2_2_bn = nn.BatchNorm3d(128, eps=0.001)
        self.pool2 = nn.MaxPool3d((1,2,2), stride=(1,2,2))
        self.conv3_1 = nn.Conv3d(128, 256, 3, padding=1, bias=bias)
        self.conv3_1_bn = nn.BatchNorm3d(256, eps=0.001)
        self.conv3_2 = nn.Conv3d(256, 256, 3, padding=1, bias=bias)
        self.conv3_2_bn = nn.BatchNorm3d(256, eps=0.001)
        self.conv3_3 = nn.Conv3d(256, 256, 3, padding=1, bias=bias)
        self.conv3_3_bn = nn.BatchNorm3d(256, eps=0.001)
        self.pool3 = nn.MaxPool3d((1,2,2), stride=(1,2,2))
        self.encdrop3 = nn.Dropout(p=0.5)
        self.conv4_1 = nn.Conv3d(256, 512, 3, padding=1, bias=bias)
        self.conv4_1_bn = nn.BatchNorm3d(512, eps=0.001)
        self.conv4_2 = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv4_2_bn = nn.BatchNorm3d(512, eps=0.001)
        self.conv4_3 = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv4_3_bn = nn.BatchNorm3d(512, eps=0.001)
        self.pool4 = nn.MaxPool3d((1,2,2), stride=(1,2,2))
        self.encdrop4 = nn.Dropout(p=0.5)
        self.conv5_1 = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv5_1_bn = nn.BatchNorm3d(512, eps=0.001)
        self.conv5_2 = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv5_2_bn = nn.BatchNorm3d(512, eps=0.001)
        self.conv5_3 = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv5_3_bn = nn.BatchNorm3d(512, eps=0.001)
        self.pool5 = nn.MaxPool3d((1,2,2), stride=(1,2,2))
        self.encdrop5 = nn.Dropout(p=0.5)

        #Decoder
        self.upconv5 = nn.ConvTranspose2d(512, 512, 4, padding=1, stride=2, bias=False)
        if self.cc5_enabled:
            self.conv5_3_D = nn.Conv3d(1024, 512, 3, padding=1, bias=bias)
        else:
            self.conv5_3_D = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv5_3_D_bn = nn.BatchNorm3d(512, eps=0.001)
        self.conv5_2_D = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv5_2_D_bn = nn.BatchNorm3d(512, eps=0.001)
        self.conv5_1_D = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv5_1_D_bn = nn.BatchNorm3d(512, eps=0.001)
        self.decdrop5 = nn.Dropout(p=0.5)

        self.upconv4 = nn.ConvTranspose2d(512, 512, 4, padding=1, stride=2, bias=False)
        if self.cc4_enabled:
            self.conv4_3_D = nn.Conv3d(1024, 512, 3, padding=1, bias=bias)
        else:
            self.conv4_3_D = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv4_3_D_bn = nn.BatchNorm3d(512, eps=0.001)
        self.conv4_2_D = nn.Conv3d(512, 512, 3, padding=1, bias=bias)
        self.conv4_2_D_bn = nn.BatchNorm3d(512, eps=0.001)
        self.conv4_1_D = nn.Conv3d(512, 256, 3, padding=1, bias=bias)
        self.conv4_1_D_bn = nn.BatchNorm3d(256, eps=0.001)
        self.decdrop4 = nn.Dropout(p=0.5)

        self.upconv3 = nn.ConvTranspose2d(256, 256, 4, padding=1, stride=2, bias=False)
        if self.cc3_enabled:
            self.conv3_3_D = nn.Conv3d(512, 256, 3, padding=1, bias=bias)
        else:
            self.conv3_3_D = nn.Conv3d(256, 256, 3, padding=1, bias=bias)
        self.conv3_3_D_bn = nn.BatchNorm3d(256, eps=0.001)
        self.conv3_2_D = nn.Conv3d(256, 256, 3, padding=1, bias=bias)
        self.conv3_2_D_bn = nn.BatchNorm3d(256, eps=0.001)
        self.conv3_1_D = nn.Conv3d(256, 128, 3, padding=1, bias=bias)
        self.conv3_1_D_bn = nn.BatchNorm3d(128, eps=0.001)
        self.decdrop3 = nn.Dropout(p=0.5)

        self.upconv2 = nn.ConvTranspose2d(128, 128, 4, padding=1, stride=2, bias=False)
        if self.cc2_enabled:
            self.conv2_2_D = nn.Conv3d(256, 128, 3, padding=1, bias=bias)
        else:
            self.conv2_2_D = nn.Conv3d(128, 128, 3, padding=1, bias=bias)
        self.conv2_2_D_bn = nn.BatchNorm3d(128, eps=0.001)
        self.conv2_1_D = nn.Conv3d(128, 64, 3, padding=1, bias=bias)
        self.conv2_1_D_bn = nn.BatchNorm3d(64, eps=0.001)

        self.upconv1 = nn.ConvTranspose2d(64, 64, 4, padding=1, stride=2, bias=False)
        if self.cc1_enabled:
            self.conv1_2_D = nn.Conv3d(128, 64, 3, padding=1, bias=bias)
        else:
            self.conv1_2_D = nn.Conv3d(64, 64, 3, padding=1, bias=bias)
        self.conv1_2_D_bn = nn.BatchNorm3d(64, eps=0.001)
        self.conv1_1_D = nn.Conv3d(64, self.output_dims, 3, padding=1, bias=bias)
        self.conv1_1_D_bn = nn.BatchNorm3d(self.output_dims, eps=0.001)
    ...
    def forward(self, x):
        ...
        x = self.upconv5(x)
        x =x.view(int(x.shape[0]/10),x.shape[1],10,x.shape[2],x.shape[3])
        if self.cc5_enabled:
            x = torch.cat([x, cc5], 1)
...
# print(DDFFAutoEncoder(1,False, False, True, False, False))